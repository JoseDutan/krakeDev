calcularValorTotal = function () {
    ...
    if (esPrecioValido("lblCampo3", precioProducto) & esCantidadValido("lblCampo2", cantidad) & esCaracterValido("lblCampo1", nombreProducto)) {
        ...
        /*
                Caso de prueba: 
                    precioProducto: 54  cantidad: 10 descuento: 10
                        --valorSubtotal: 54
                        --descuento: 5.4
...
esCantidadValido = function (idComponenteError, cantidad) {
    let hayErrores = false;
    if (isNaN(cantidad)) {
        mostrarTexto(idComponenteError, "CAMPO OBLIGATORIO");
        hayErrores = true;
...
esCaracterValido = function (idComponenteError, producto) {
    let hayErrores = false;
    if (!isNaN(producto)) {
        mostrarTexto(idComponenteError, "CAMPO OBLIGATORIO");
        hayErrores = true;
    ...
    if ((producto.lenght > 10)) {
        mostrarTexto(idComponenteError, "DEBE DE TENER 10 CARACTERES");
        hayErrores = true;
    }
    if (hayErrores == false) {
        mostrarTexto(idComponenteError, "")
